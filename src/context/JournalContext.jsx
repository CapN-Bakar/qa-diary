...
export function JournalProvider({ children }) {
  ...
  function mapToDB(entry, isNew = false) {
    return {
      id: entry.id,
      date: entry.date,
  ...
  const unlock = useCallback(async (p1, p2) => {
    ...
    if (h1 !== OWNER_PIN1_HASH)
      return { ok: false, error: "First PIN is incorrect." };
    if (h2 !== OWNER_PIN2_HASH)
      return { ok: false, error: "Second PIN is incorrect." };
    setUnlocked(true);
    return { ok: true };
  }, []);
